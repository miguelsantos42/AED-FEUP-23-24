...
bool Graph<T>::removeEdge(const T &sourc, const T &dest) {
    ...
    Vertex<T> *bsourc = findVertex(sourc);
    Vertex<T> *bdest = findVertex(dest);
    for(Edge<T> i: bsourc->adj){
        if(i.getDest()->info == dest){
            bsourc->removeEdgeTo(bdest);
            return true;
        }
    }
    return false;
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    //procuro o vertice com info in
    Vertex<T> *newvertice = findVertex(in);

    //caso não haja retorno falso
    if(newvertice == nullptr){
        return false;
    }

//caso haja:
    //removo todas as conexões ou arestas que existam com este vertice com esta info
    for(auto i : vertexSet){
        i->removeEdgeTo(newvertice);
    }

    //removo o próprio vertice
